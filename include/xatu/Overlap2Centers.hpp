...
class Overlap2Centers : public virtual IntegralsBase {
    ...
    //// Methods
    public:
        Overlap2Centers(const GTFConfiguration&, const uint32_t nR, const std::string& o2Mat_name = "o2Mat", const bool comp = true);
        Overlap2Centers(const IntegralsBase&, const uint32_t nR, const std::string& o2Mat_name = "o2Mat_AUX", const bool basis_id = false); 
        ~Overlap2Centers(){};
    ...
    void overlap2Cfun(const uint32_t nR, const std::string& o2Mat_name, const bool basis_id = false);
