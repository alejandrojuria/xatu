...
pair */
cx_vec ehPairCoefs(double energy, const vec& kpoints, const vec& gapEnergy){
    ...
    cout << "Selected k: " << kpoints(closestK) << "\t" << closestK << endl;
    cout << "Closest gap energy: " << gapEnergy(closestK) << endl;
    // By virtue of band symmetry, we expect n < nk/2
    int nk = kpoints.n_elem;
    coefs(closestK) = 1./sqrt(2);
    coefs(nk - closestK) = 1./sqrt(2);

    cout << "Energy gap (-k): " << gapEnergy(closestK) << endl;
    cout << "Energy gap (k): " << gapEnergy(nk - closestK) << endl;
...
double fermiGoldenRule(const cx_vec& initialCoefs, double initialE, int nbands, double Q, 
    ...
    cout << arma::cdot(ehCoefs, W*initialCoefs) << endl;
    transitionRate = 2*PI*pow(abs(arma::cdot(ehCoefs, W*initialCoefs)),2)*rho/hbar;
