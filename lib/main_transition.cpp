...
int main(){
    ...
    std::string filename = "transition_ribbon_4_300_noonsite_dos_all";
    ...
    for(int n = 0; n < (int)NArray.n_elem; n++){
        ...
        cout << "Right" << endl;
        exciton = Exciton(N, Ncell, Q, nBulkBands, nEdgeBands, vec{1, 1});
        ...
        exciton = Exciton(N, Ncell, Q, nBulkBands, nEdgeBands, vec{0, 1});
        transitionRate = exciton.fermiGoldenRule(initialCoefs, initialEnergy);
