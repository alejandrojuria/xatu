...
extern "C" {
    void skubo_w_(int* nR, int* norb, int* norb_ex, int* nv, int* nc, double* nRvec, double* bravaisLattice, double* motif, 
                  double* hhop, double* shop, int* nk, double* rkx, double* rky, double* rkz, std::complex<double>* fk_ex, double* e_ex);
}
...
int main(int argc, char* argv[]){
    ...
    arma::mat nRvec = bulkExciton.unitCellList;
    ...
    arma::cube shop = arma::real(bulkExciton.overlapMatrices);
    ...
    skubo_w_(&nR, &norb, &norb_ex, &nv, &nc, nRvec.memptr(), R.memptr(), B.memptr(), hhop.memptr(), shop.memptr(), &nk, rkx.memptr(),
             rky.memptr(), rkz.memptr(), eigvec.memptr(), rkz.memptr());
}
