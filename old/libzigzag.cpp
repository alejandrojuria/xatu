<<<<<<< Updated upstream:old/libzigzag.cpp
#include <armadillo>
#include <complex>
#include <math.h>
#include <stdlib.h>
...
mat createMotiv(int N){
	mat motiv(3, 2*(N+1));
	motiv.col(0) = arma::vec({0,0,0});
	motiv.col(1) = n1;
	motiv.col(2) = a1;
	motiv.col(3) = a1 + n2;
	...
	for (int n = 2; n <= N; n++){
		if(n%2 == 0){
			motiv.col(2*n) = (a1 + a2)*n/2;
			motiv.col(2*n + 1) = (a1 + a2)*n/2 + n1;
		}
		else{
			motiv.col(2*n) = (a1 + a2)*(n-1)/2 + a1;
			motiv.col(2*n + 1) = (a1 + a2)*(n-1)/2 + motiv.col(3);
		};
	}
	/*for(int n = 0; n < (int)motiv.n_cols; n++){
		motiv.col(n) -= arma::vec({0, -N})
	}; */
...
void initializeBlockMatrices() {
	...
	Mzeeman = arma::zeros(8,8);
	Mzeeman.diag() = arma::vec({1., -1., 1., 1., 1., -1., -1., -1.,})*zeeman;
...
cx_mat inversionOperator(const cx_vec& eigenvector, int N){
...
double expectedSpinZValue(const arma::cx_vec& eigvec, int N){
...
/* Routine to calculate the expected value of the spin projection
Sz of a TB eigenstate. */
double expectedSpinYValue(const arma::cx_vec& eigvec, int N){
...
/* Routine to calculate the expected value of the spin projection
Sz of a TB eigenstate. */
double expectedSpinXValue(const arma::cx_vec& eigvec, int N){
...
double densityOfStates(double energy, double delta, const mat& energies){
...
void writeDensityOfStates(const mat& energies, double delta, FILE* dosfile){
...
>>>>>>> Stashed changes:lib/libzigzag.cpp
