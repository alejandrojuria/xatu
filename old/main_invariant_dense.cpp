...
cx_mat spinOperator;
cx_mat PpTB_matrix, PmTB_matrix, fullTB_matrix;
...
int main(){
	...
	cx_mat eX = positionMatrix(0, motif, nOrbitals, Ncell, N);
	cout << "exp(X) computed " << endl;
	cx_mat eY = positionMatrix(1, motif, nOrbitals, Ncell, N);
	cout << "exp(Y) computed " << endl;

	cx_mat sectorPlus = projectorMatrix(0, fullTB_matrix, 
												PpTB_matrix, Ncell);
	cout << "Projector + computed" << endl;

	cx_mat sectorMinus = projectorMatrix(1, fullTB_matrix, 
												PmTB_matrix, Ncell);
	cout << "Projector - computed" << endl;

	double bottPlus = sectorBottIndex(eX, eY, sectorPlus);
	cout << "Bott index + computed: " << bottPlus << endl;
	double bottMinus = sectorBottIndex(eX, eY, sectorMinus);
	cout << "Bott index - computed: " << bottMinus << endl;

	double spinBott = (bottPlus - bottMinus)/2;
