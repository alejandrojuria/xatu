...
int main(){
    ...
    std::string filename = "transition_diff";
    ...
    /*int firstNcell = 10;
    vec NcellArray = zeros(5);
    NcellArray(0) = firstNcell;
    for(int i = 1; i < 5; i++){
        NcellArray(i) = NcellArray(i-1) + NcellArray(i-1)*2 - 1;
    };
    cout << NcellArray << endl;*/
    //vec NcellArray = arma::regspace(10, 200);
    vec NcellArray = arma::regspace(10, 150);
    for(int n = 0; n < (int)NcellArray.n_elem; n++){
        ...
        mat states = createBasis(N, Q, kpoints, nBulkBands, 0); // No edge states for bulk exciton
