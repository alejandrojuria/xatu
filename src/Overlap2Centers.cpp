 ...
 */
Overlap2Centers::Overlap2Centers(const GTFConfiguration& GTFConfig, const std::string& o2Mat_name, const bool comp) : IntegralsBase{GTFConfig} {

    if(comp){
        overlap2Cfun(ncells, o2Mat_name);
    }
 ...
 */
Overlap2Centers::Overlap2Centers(const IntegralsBase& IntBase, const std::string& o2Mat_name, const bool basis_id) : IntegralsBase{IntBase} {

    overlap2Cfun(ncells, o2Mat_name, basis_id);
...
auto begin = std::chrono::high_resolution_clock::now();  
    ...
    for(int Rind : R_unpaired){ //compute the upper triangles of the R-matrices corresponding to unpaired Bravais vectors 
        for(long int s = 0; s < nelem_triang_offd; s++){
            ...
            int m_ket  {orbitals_info_int[orb_ket][3]};
            int nG_ket {orbitals_info_int[orb_ket][4]};
            arma::colvec coords_ket {RlistAU.col(Rind) + arma::colvec{orbitals_info_real[orb_ket][0], orbitals_info_real[orb_ket][1], orbitals_info_real[orb_ket][2]} }; 
            std::vector<int> g_coefs_ket   {g_coefs.at( L_ket*(L_ket + 1) + m_ket )};
